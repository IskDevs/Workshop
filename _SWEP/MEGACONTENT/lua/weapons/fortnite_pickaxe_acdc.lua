...
FORTNITE ACDC 2019
---------------------------------*/
AddCSLuaFile( "fortnite_pickaxe_acdc.lua" )

SWEP.PrintName = "AC/DC"
...
SWEP.ViewModel = "models/fortnite/v_fbr_acdc.mdl" 
SWEP.WorldModel = "models/fortnite/w_fbr_acdc.mdl"
...
if (CLIENT) then
SWEP.WepSelectIcon = surface.GetTextureID( "vgui/entities/fortnite_pickaxe_acdc" )
killicon.Add( "fortnite_pickaxe_acdc", "VGUI/hud/fortnite_pickaxe_acdc", color_white )
end
...
function SWEP:Deploy()
	...
	self:EmitSound("Fortnite_ACDC.Draw")
...
self:EmitSound("Fortnite_ACDC.Swing_" .. math.random(1, 2) .. "")
self:MeleePickaxe()
end
...
self:StopSound("Fortnite_ACDC.Draw")
end
...
timer.Stop( "LightningEffect" .. self:EntIndex() ) 
timer.Stop( "LightningEffectTwo" .. self:EntIndex() ) 
return true
end

end

function SWEP:Lightning()
timer.Create( "LightningEffect" .. self:EntIndex(), 0.2, 1, function() self:Lightning() end ) 
...
util.Effect( "fortnite_electric_sword", effectdata )
end

function SWEP:LightningTwo()
timer.Create( "LightningEffectTwo" .. self:EntIndex(), 0.2, 1, function() self:LightningTwo() end ) 
local effectdata = EffectData()
effectdata:SetOrigin( self.Owner:GetShootPos() )
effectdata:SetStart( self.Owner:GetShootPos() )
...
timer.Simple( 1, function() if IsValid(self) and IsValid(self.Owner) then self.Owner:Give("fortnite_pickaxe_acdc_swep_ai") SafeRemoveEntity(self) end end )
end
return true
end
...
if SERVER then
	...
	local idleEffect = ents.Create("light_dynamic")
	idleEffect:SetColor(Color(0,255,255,255))
	idleEffect:SetKeyValue("brightness", "3")
	idleEffect:SetKeyValue("distance", "100")
	idleEffect:SetPos(self:GetPos())
	idleEffect:SetParent(self)
	idleEffect:Spawn()
	idleEffect:Activate()
	idleEffect:Fire("TurnOn", "", 0)
	idleEffect:Fire("TurnOff","",0.1)
	idleEffect:Fire("Kill", "", 0.1)
	self:DeleteOnRemove(idleEffect)
end
